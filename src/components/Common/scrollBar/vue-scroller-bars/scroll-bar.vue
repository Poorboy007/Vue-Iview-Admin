<template>
  <div ref="wraper" :class="wraperClasses" @DOMMouseScroll="handleDOMMouseWheel" @mousewheel="handleMouseWheel">
    <div ref="content" :class="`${prefix}-content`" :style="contentStyles">
      <slot />
    </div>
    ...
    <div v-show="percentX < 1 && percentY < 1" :class="`${prefix}-place-holder`" />
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    handleMouseWheel(e) {
      this.scrollOffsetY += this.percentY < 1 ? e.deltaY / 3.5 : 0
      this.scrollOffsetX += this.percentX < 1 ? e.deltaX / 3.5 : 0
      if (this.percentY < 1) {
